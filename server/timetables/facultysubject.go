...
func GetFacultySubjects(semesterID int, academicYearID int, sectionID int) ([]models.FacultySubject, error) {
	...
	rows, err := config.Database.Query("SELECT faculty_id, subject_id,semester_id,department_id,academic_year_id,section_id,classroom_id FROM faculty_subjects WHERE  semester_id = ? && academic_year_id = ? && section_id = ?", semesterID, academicYearID, sectionID)
	if err != nil {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&fs.FacultyID, &fs.SubjectID, &fs.SemesterID, &fs.DepartmentID, &fs.AcademicYear, &fs.SectionID, &fs.ClassroomID); err != nil {
